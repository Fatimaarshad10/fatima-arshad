                        ...
                        {blogs.map((blog, idx) => (
                            <div key={idx} className="bg-dark-800 rounded-xl border border-dark-700 p-8 hover:border-primary-500/50 transition-all duration-300">
                                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-2">
                                    <h2 className="text-2xl font-semibold text-white mb-2 md:mb-0">{blog.title}</h2>
                                    <span className="text-sm text-primary-400">{blog.date}</span>
                                </div>
                                <p className="text-gray-300 mb-4">{blog.excerpt}</p>
                                <Link href={`/blog/${blog.slug}`} className="text-primary-400 hover:text-primary-300 font-medium transition-colors duration-200">
                                    Read More &rarr;
                                </Link>
                            </div>
                        ))}
